...
(function () {
    ...
    var CELL_HEIGHT = 39 * (32/39),
        CELL_WIDTH = 41 * (32/39),

        gCellHeight = CELL_HEIGHT,
        gCellWidth = CELL_WIDTH,

        MOLECULE_CELL_HEIGHT = 39 * (32/39),
        MOLECULE_CELL_WIDTH = 41 * (32/39),
    ...
    function replay_moves(moveList, grid) {
        ...
        for (moveIdx = 0; moveIdx < moveList.length; moveIdx += 1) {
            ...
            _assert(atom !==  '.' && atom !== '#', 'Kein Atom vorhanden');

            data = grid[row];

            if (startRow === endRow) {
                _assert(startCol !== endCol, 'Keine Schritte')
                if (endCol < startCol) {
                ...
                _assert(startCol === endCol, 'Diagonal geht nicht')
                if (endRow < startRow) {
            ...
            _assert(row === endRow && col === endCol, 'Der Weg ist versperrt');
    ...
    function onCompleteLevel() {
        show_success_dialog(
            format(
                "Du hast diesen Level in <b>\f</b> Schritten geschafft.",
                gg.history.length
            )
        );
    }
    ...
    function show_about_dialog(fnCallback) {
    ...
    function init() {
        ...
        dlgAbout = kp_ModalDialog('about-dialog');
